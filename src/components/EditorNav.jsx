...
function EditorNav() {
  ...
  const positionIndicator = (id, animate) => {
    ...
    if (animate && !prefersReducedMotion()) {
      gsap.to(indicator, { left, width, duration: 0.45, ease: "power3.out" });
    } else {
  ...
  useEffect(() => {
    positionIndicator(active, hasPositioned.current);
    hasPositioned.current = true;
  }, [active]);

  useEffect(() => {
    const handleResize = () => positionIndicator(active, false);
    ...
    >
      <div className="mx-auto max-w-6xl rounded-xl border border-[var(--border-soft)] bg-[var(--chrome)] backdrop-blur-md shadow-[0_4px_24px_rgba(0,0,0,0.35)]">
        <div className="flex items-center gap-4 px-3 sm:px-4 h-14">
          ...
          {/* tabs */}
          <ul ref={tabListRef} className="relative hidden md:flex items-stretch h-14 -mb-px overflow-x-auto">
            {TABS.map((tab) => (
              <li key={tab.id}>
                <button
                  ref={(el) => {
                  ...
                  data-cursor-hover
                  onClick={() => goTo(tab.id)}
                  className={`h-14 px-4 flex items-center gap-2 font-mono text-[13px] whitespace-nowrap transition-colors cursor-pointer ${
                    active === tab.id
                      ? "text-[var(--text)]"
            ...
            <span
              ref={indicatorRef}
              className="absolute left-0 bottom-0 w-0 h-[2px] bg-[var(--signal)] rounded-full pointer-events-none"
            />
          </ul>
